...
void deleteBatch(){
    ...
    int index = 1;
    for(auto &batchEntry : inventory[productName].batchList)
        cout << index++ << ". " << batchEntry.quantity
    ...
    inventory[productName].batchList.erase(
        inventory[productName].batchList.begin() + (choice - 1)
    );

    if(inventory[productName].batchList.empty())
        inventory.erase(productName);
